...
function sphericalEdges(data, parameters) {
    ...
    function generateEdges(resolution, localVertices) {
        ...
        var ratios = [];
        const ca = VF.vectorDot(localVertices[data.edges[0][0]].hypersphere, localVertices[data.edges[0][1]].hyperssphere);
    ...
    function inSphericalFace(point, face) {
        ...
        return pointInPolygon(flatPoint, faces[face].polygon);
    ...
    function visibleEdges(invisibleLines, camera, spheres, vertices) {
...
function generateSpheres(data, vertices) {
    ...
    if (data.metric === "h" || data.metric === "p" || data.metric === "u") {
        ...
        var u1, u2, u3, v1, v2, v3, d, triCenterStereo, center, radius, sphereDict, normal, type, center4;
    ...
    for (var i = 0; i < data.numFaces; i++) {
        ...
        if (data.metric === "h" || data.metric === "p" || data.metric === "u") {
            ...
            curve.forEach((vert) => {
                ...
                drawVerts.push([vert, pointInSphericalPolygon(vert, spheres)]);
...
function pointInSphericalPolygon(point, spheres) {
    ...
    const p = VF.vectorScale(point, d / VF.vectorDot(spheres))
